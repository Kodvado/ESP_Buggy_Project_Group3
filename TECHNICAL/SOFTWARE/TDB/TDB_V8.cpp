...
protected:
    ...
    Ticker getValue;
    Timeout readStopTimeout;                                                            ///< Timer for triggering readings.
...
public:
    ...
    void toggle3and3Read(void)
    {
        if (readOff)
        {
            getValue.attach_us(callback(this, &SensorArray::read3and3), 500);
            readOff = false;
        }
        else
        {
                    ...
                    if (sample4Val < 0.37){
                        if (sample2Val < 0.37){
                            if (sample5Val < 0.37){
                                if (sample1Val < 0.37){
                                    if (sample6Val < 0.37){
                                        readStopTimeout.attach(callback(this, &SensorArray::readStopTest), 0.5);
                                    }
                                }
                            }
                        }
...
volatile float RM_Kp = 0.000012; // Proportional gain of right motor
...
volatile float S_Kp = 125; // Proportional gain of steering
volatile float S_Ki = 0;  // Integral gain of steering
volatile float S_Kd = 0.011;  // Derivative gain of steering
...
void steeringControl()
{
    S_PID_output = PID_controller(sennyArray.getErrorValue(), integral_steering, previous_error_steering, 0.001, S_Kp, S_Ki, S_Kd);
