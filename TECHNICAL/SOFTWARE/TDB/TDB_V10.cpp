...
volatile float S_Kp = 400; // Proportional gain of steering
...
public:
    ...
    void read3and3(void)
    {
        ...
        case 3:
            ...
            if ((readStopState) && (sample1Val < (s1MinVal + 0.4 )) && (sample2Val < (s2MinVal + 0.4 )) && (sample3Val < (s3MinVal + 0.4 )) && (sample4Val < (s4MinVal + 0.4 )) && (sample5Val < (s5MinVal + 0.4 )) && (sample6Val < (s6MinVal + 0.4 )))
            {
                motorStop();
                motorcontrolstate = false;
            }
    ...
    {
        return ((7 * sample1Val) + (3.5 * sample2Val) + sample3Val) - (sample4Val + (3.5 * sample5Val)+(7 * sample6Val));
    }
...
void steeringControl()
{
    ...
    float targetRPPSdiff = abs(R_targetPPS/R_defaultPPS);
    if (targetRPPSdiff <= 0.7 || targetRPPSdiff >= 1.3)
    {
        dropGear();
    }
}
