...
float L_defaultPPS = 30000; ///< Target PPS for left motor
float L_targetPPS;
float R_defaultPPS = 30000; ///< Target PPS for right motor
float R_targetPPS;
float L_PWM = 0.0;                  ///< Default PWM for left motor
float R_PWM = 0.0;                  ///< Default PWM for right motor
...
float S_Kp = 100; // Proportional gain of steering
float S_Ki = 0;   // Integral gain of steering
float S_Kd = 0;   // Derivative gain of steering 

float integral_steering = 0;       ///< Integral term for steering PID controller
float previous_error_steering = 0; ///< Previous error for steering PID controller
...
{
    updateEncoders();
    float leftSpeed = LeftPulse * (1 / dt);
    float rightSpeed = RightPulse * (1 / dt);
}
...
{
    calculateSpeed();
    float leftError = L_targetPPS - LeftPulse;
    float rightError = R_targetPPS - RightPulse;

    L_PWM += PID_controller(leftError, integral_left, previous_error_left, dt, LM_Kp, LM_Ki, LM_Kd);
    R_PWM += PID_controller(rightError, integral_right, previous_error_right, dt, RM_Kp, RM_Ki, RM_Kd);
